...
Window {
    ...
    Row {
        ...
        function allDie() {
            return diedCount == children.lenghth;
        }
        ...
        ParallelAnimation {
            id: animateBullet;
            property var fire: 0;
            property var direBubble: -1;

            function caculateDiedBubble() {
                var crossX = turret.rotation == 0 ? (root.width/2 - bullet.width/2) :
                                                    (root.width/2 - bullet.width/2) + (15 - (root.height - bullet.height/2)) /
                ...
                for(diedBubble=-1; i < bubbles.children.length; i++) {
                    bubble = bubbles.children[i];
                    if(crossX > bubbles.x + bubble.x && crossX < bubbles.x + bubbles.x + bubble.width) {
            ...
            onStopped: {
                if(fire == 1) {
                    ...
                    if(root.requestReset == true) {
                        ...
                        playGame.enable = true;
                    }
                }
            }

            function fire() {
                if(playGame.enable || animateBullet.fire != 0) {
                    return;
                }
                bullet.visible = true;
                ...
                if(turret.rotation == 0) {
                    animateBulletX.to = bullet.x;
                } else {
                    animateBulletX.to = (root.width/2 - bullet.width/2) -
                                        (root.height - bullet.height/2) /
                                        Math.tan(90 + turret.rotation) * 0.017453293;
